...
def inject_user():
    ...
    return dict(username=None, role=None)
    ...
    if "username" in session:
        if request.method == "POST":
            ...
            try:
                ...
                conn.close()
            flash("Rent request submited succesfuly!", "success")
            return render_template("book.html")  # ADD A LIST OF USERS RENTS!!!!
...
def reservations():
    ...
    if "username" in session and session["role"] == "admin":
        ...
        if result:
            ...
            with conn.cursor() as cursor:
                query = "SELECT * FROM reservation WHERE school_id = %s"
